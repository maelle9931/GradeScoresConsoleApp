        ...
        public void ImportScoresFromFile_WithValidInput_ReturnsGraderWithScoreList()
        {
            ...
            using (var stream = new MemoryStream(Encoding.UTF8.GetBytes(sb.ToString())))
            {
                ...
                using (var streamReader = new StreamReader(stream))
                {
                    ...
                    Assert.AreEqual(FIRST_LINE, grader.ScoreList[0].LastName + SEPARATOR + grader.ScoreList[0].FirstName + SEPARATOR + grader.ScoreList[0].Score, false);

                    //second item should be equal to the second line
                    Assert.AreEqual(SECOND_LINE, grader.ScoreList[1].LastName + SEPARATOR + grader.ScoreList[1].FirstName + SEPARATOR + grader.ScoreList[1].Score, false);
                }
            }
        }
